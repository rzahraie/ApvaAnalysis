...
def survival_rows(bars: List[Bar]) -> Dict[Tuple[str, str], Dict[str, float]]:
    counts: Counter[Tuple[str, str]] = Counter(); s1=Counter(); s3=Counter(); s5=Counter()
    for i,b in enumerate(bars):
        k=(b.state,b.age_bucket); counts[k]+=1
        for lag,c in ((1,s1),(3,s3),(5,s5)):
            if i+lag < len(bars) and bars[i+lag].state == b.state:
                c[k]+=1
    out={}
    for k,n in counts.items():
        a=s1[k]/n if n else 0.0; b=s3[k]/n if n else 0.0; c=s5[k]/n if n else 0.0
        out[k]={"count":n,"survive1":a,"hazard1":1-a,"survive3":b,"hazard3":1-b,"survive5":c,"hazard5":1-c}
    return out


def write_instrument(r: InstrumentResult, out_root: str) -> None:
    out_dir=os.path.join(out_root,r.instrument); ensure_dir(out_dir)
    path=os.path.join(out_dir,f"StateHazard_{r.instrument}.txt")
    rows=survival_rows(r.bars)
    with open(path,'w',encoding='utf-8') as f:
        f.write(f"APVA State Hazard Function Study v0.1 - {r.instrument}\n")
        f.write('='*62+'\n\n')
        f.write(f"Rows: {len(r.bars)}\n\n")
        f.write(f"{'State':<28} {'Age':<6} {'N':>8} {'Surv1':>9} {'Haz1':>9} {'Surv3':>9} {'Haz3':>9} {'Surv5':>9} {'Haz5':>9}\n")
        for st in STRUCTURAL_STATES:
            for age in AGE_BUCKETS:
                x=rows.get((st,age))
                if x:
                    f.write(f"{st:<28} {age:<6} {x['count']:8d} {pct(x['survive1']):>9} {pct(x['hazard1']):>9} {pct(x['survive3']):>9} {pct(x['hazard3']):>9} {pct(x['survive5']):>9} {pct(x['hazard5']):>9}\n")


def write_aggregate(results: List[InstrumentResult], out_root: str) -> None:
    out_dir=os.path.join(out_root,'StateHazard'); ensure_dir(out_dir)
    path=os.path.join(out_dir,'StateHazard_All.txt')
    instruments=[r.instrument for r in results]
    maps={r.instrument: survival_rows(r.bars) for r in results}
    with open(path,'w',encoding='utf-8') as f:
        f.write('APVA State Hazard Function Study v0.1 - Aggregate\n')
        f.write('='*58+'\n\n')
        f.write('Instruments: '+', '.join(instruments)+'\n\n')
        f.write(f"{'State':<28} {'Age':<6}")
        for inst in instruments:
            f.write(f" {inst+'_N':>8} {inst+'_Haz1':>10} {inst+'_Haz3':>10} {inst+'_Haz5':>10}")
        f.write(f" {'ValidN':>7} {'MeanHaz1':>10} {'MeanHaz3':>10} {'MeanHaz5':>10}\n")
        ranked=[]
        for st in STRUCTURAL_STATES:
            for age in AGE_BUCKETS:
                v1=[]; v3=[]; v5=[]; valid=0; line=f"{st:<28} {age:<6}"
                for inst in instruments:
                    row=maps[inst].get((st,age))
                    if row:
                        valid+=1; v1.append(row['hazard1']); v3.append(row['hazard3']); v5.append(row['hazard5'])
                        line += f" {row['count']:8d} {pct(row['hazard1']):>10} {pct(row['hazard3']):>10} {pct(row['hazard5']):>10}"
                    else:
                        line += f" {0:8d} {pct(0):>10} {pct(0):>10} {pct(0):>10}"
                if valid>=2:
                    line += f" {valid:7d} {pct(mean(v1)):>10} {pct(mean(v3)):>10} {pct(mean(v5)):>10}\n"; f.write(line)
                    ranked.append((mean(v1),st,age))
        f.write('\nRankings\n========\n\nHighest replicated t+1 hazard\n.............................\n')
        for i,(hz,st,age) in enumerate(sorted(ranked, reverse=True)[:50],1):
            f.write(f"{i:3d}. {st} Age={age} | Hazard1={pct(hz)}\n")
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    print(f"Wrote StateHazard reports under {args.out_root}")
    return 0
