#!/usr/bin/env python3
"""
APVA cross-instrument transfer test.

Purpose
-------
Test whether the current ES-derived APVA topology transfers to other instruments.
...
Policy:
    ...
    optional wide disaster stop, default 20 raw points/ticks/units in dataset scale
    no profit cap

Important caveat
----------------
The disaster stop is in raw dataset units. Cross-instrument comparability may be
poor unless returns are normalized. The script therefore reports both raw policy
outcomes and SignedNormalizedReturn where available.

Recommended Jupyter usage
-------------------------

    import runpy
    ns = runpy.run_path("scripts/apva_cross_instrument_transfer.py", run_name="not_main")
    ns["main"]([
        "--input", "tables/apva_forward_signed_return_dataset_v1.csv",
        "--outdir", "outputs/cross_instrument_transfer_compression_precursor",
        "--exit-horizon", "5",
        "--disaster-stop", "20"
    ])
...
Outputs
-------
- cross_instrument_entries.csv
- cross_instrument_policy_rows.csv
- cross_instrument_summary.csv
- cross_instrument_week_summary.csv
- cross_instrument_scorecard.csv
"""
...
def prepare_df(args: argparse.Namespace) -> pd.DataFrame:
    ...
    numeric = [
        ...
        "DirectionalMFE", "DirectionalMAE", "DirectionalNormalizedMFE", "DirectionalNormalizedMAE",
        "RollingEntropy", "RollingDirectionalPresence", "RollingVelocity",
        "DominantPressureValue", "DirectionSign",
    ]
    for c in numeric:
        if c in df.columns:
...
def find_entries(df: pd.DataFrame, args: argparse.Namespace) -> pd.DataFrame:
    ...
    if args.instruments:
        allowed = {x.strip().upper() for x in args.instruments.split(",") if x.strip()}
        state &= df["Instrument"].astype(str).str.upper().isin(allowed)
    ...
    if out.empty:
        raise RuntimeError("No cross-instrument entries found")
    return out.reset_index(drop=True)


def apply_policy(df: pd.DataFrame, entries: pd.DataFrame, args: argparse.Namespace) -> pd.DataFrame:
    lookup = {(r.Instrument, r.File, int(r.BarIndex)): r for r in df.itertuples(index=False)}
    rows = []
    for eid, e in entries.iterrows():
        ...
        for offset in range(0, args.exit_horizon + 1):
            hit = lookup.get((instrument, file, entry_bar + offset))
            if hit is None:
                continue
        ...
        rows.append({
            ...
            "SignedReturnAtEntry": float(e.get("SignedReturn", np.nan)),
            "SignedNormalizedReturnAtEntry": float(e.get("SignedNormalizedReturn", np.nan)) if "SignedNormalizedReturn" in e.index else np.nan,
            "DirectionalMFEAtEntry": float(e.get("DirectionalMFE", np.nan)) if "DirectionalMFE" in e.index else np.nan,
            "DirectionalMAEAtEntry": float(e.get("DirectionalMAE", np.nan)) if "DirectionalMAE" in e.index else np.nan,
            "DirectionalNormalizedMFEAtEntry": float(e.get("DirectionalNormalizedMFE", np.nan)) if "DirectionalNormalizedMFE" in e.index else np.nan,
            "DirectionalNormalizedMAEAtEntry": float(e.get("DirectionalNormalizedMAE", np.nan)) if "DirectionalNormalizedMAE" in e.index else np.nan,
...
def group_summary(rows: pd.DataFrame, group_cols: list[str]) -> pd.DataFrame:
    ...
    for key, g in rows.groupby(group_cols, dropna=False, sort=True):
        ...
        d.update({f"Policy_{k}": v for k, v in summarize(g["PolicyOutcome"]).items()})
        d.update({f"TimeExit_{k}": v for k, v in summarize(g["TimeExitOutcome"]).items()})
        if "SignedNormalizedReturnAtEntry" in g.columns and g["SignedNormalizedReturnAtEntry"].notna().any():
            d.update({f"SignedNormEntry_{k}": v for k, v in summarize(g["SignedNormalizedReturnAtEntry"]).items()})
...
def build_scorecard(by_inst: pd.DataFrame, rows: pd.DataFrame) -> pd.DataFrame:
    ...
    out = [
        ...
        {"Metric": "overall_policy_mean", "Value": overall["Mean"]},
        {"Metric": "overall_policy_median", "Value": overall["Median"]},
        {"Metric": "overall_policy_tstat", "Value": overall["TStat"]},
        {"Metric": "overall_policy_win_rate", "Value": overall["WinRate"]},
        {"Metric": "overall_policy_profit_factor", "Value": overall["ProfitFactor"]},
        {"Metric": "overall_stop_rate", "Value": float(rows["DisasterStopped"].mean())},
        {"Metric": "instrument_positive_fraction", "Value": float((by_inst["Policy_Mean"] > 0).mean()) if not by_inst.empty else np.nan},
        {"Metric": "instrument_median_policy_mean", "Value": float(by_inst["Policy_Mean"].median()) if not by_inst.empty else np.nan},
        {"Metric": "instrument_min_policy_mean", "Value": float(by_inst["Policy_Mean"].min()) if not by_inst.empty else np.nan},
        {"Metric": "instrument_max_policy_mean", "Value": float(by_inst["Policy_Mean"].max()) if not by_inst.empty else np.nan},
    ]
    for _, r in by_inst.sort_values("Policy_Mean", ascending=False).iterrows():
        inst = str(r["Instrument"])
        out.append({"Metric": f"{inst}_count", "Value": float(r["Policy_Count"])})
        out.append({"Metric": f"{inst}_policy_mean", "Value": float(r["Policy_Mean"])})
        out.append({"Metric": f"{inst}_policy_median", "Value": float(r["Policy_Median"])})
        out.append({"Metric": f"{inst}_policy_profit_factor", "Value": float(r["Policy_ProfitFactor"])})
        out.append({"Metric": f"{inst}_stop_rate", "Value": float(r["StopRate"])})
    return pd.DataFrame(out)
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    p.add_argument("--outdir", default="outputs/cross_instrument_transfer_compression_precursor")
    ...
    by_week = group_summary(rows, ["Instrument", "RegimeWeek"]) if "RegimeWeek" in rows.columns else pd.DataFrame()
    score = build_scorecard(by_inst, rows)

    entries.to_csv(outdir / "cross_instrument_entries.csv", index=False)
    rows.to_csv(outdir / "cross_instrument_policy_rows.csv", index=False)
    by_inst.to_csv(outdir / "cross_instrument_summary.csv", index=False)
    by_week.to_csv(outdir / "cross_instrument_week_summary.csv", index=False)
    score.to_csv(outdir / "cross_instrument_scorecard.csv", index=False)

    print("APVA cross-instrument transfer complete")
