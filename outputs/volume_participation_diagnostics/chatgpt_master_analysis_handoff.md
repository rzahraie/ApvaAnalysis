...
No explicit raw volume field or derived volume column is present in the current canonical datasets or raw NT8 state-log exports.
...
Among available continuous proxies, `PriorSlope_DominantPressureValue` has the largest absolute Spearman association with PriorSlope membership within Reference base entries (`0.240`), as expected because it defines the frozen candidate. Excluding that definitional field, the largest membership association is `RollingEntropy` at only `0.066`.

## Entropy And State Context

PriorSlope Q3 Reference entries have RollingEntropy mean `1.165` and median `1.295`; Reference base entries outside PriorSlope Q3 have entropy mean `1.139`. The difference (`0.026`) is modestly higher for PriorSlope Q3. Because the frozen base state already bounds entropy, this report treats entropy as context rather than an independent explanation.

The most common MacroState is `Unresolved` with `762` of `786` Reference entries (`0.969`). The most common SponsorState is `Unresolved` (`0.499` share), and the most common SequencePhase is `B2B` (`0.356` share).
