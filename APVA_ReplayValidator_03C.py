#!/usr/bin/env python3
"""
APVA Replay Validator 03C - Corrected Event Quality

Ranks APVA v1.0 DestinationSelection event classes by directional behavior
using corrected PriorNeutralPersistenceBars.
...
OUTPUT_TEXT = "APVA_ReplayValidator_03C.txt"
EVENT_QUALITY_CSV = "EventQualityRanking_03C.csv"
DESTINATION_QUALITY_CSV = "DestinationQuality_03C.csv"
NEUTRAL_PERSISTENCE_CSV = "NeutralPersistenceQuality_03C.csv"
INTERACTION_QUALITY_CSV = "InteractionQuality_03C.csv"
    ...
    for path in files:
        with open(path, "r", newline="", encoding="utf-8-sig") as handle:
            ...
            if reader.fieldnames is None or "PriorNeutralPersistenceBars" not in reader.fieldnames:
                raise ValueError(
                    f"PriorNeutralPersistenceBars column is missing from {path}. "
                    "Regenerate the NT8 export with the corrected xApvaV10StateMachine first."
                )
            for raw in reader:
                rows.append(
                    Row(
...
def neutral_persistence_quality(events: Sequence[Event]) -> List[Dict[str, object]]:
    ...
    return [summarize_group("PriorNeutralPersistenceGroup", group, grouped.get(group, [])) for group in order if group in grouped or group != "Unknown"]
...
def interaction_quality(events: Sequence[Event]) -> List[Dict[str, object]]:
    ...
    for key in sorted(grouped_persistence_destination, key=lambda item: (str(item[0]), str(item[1]))):
        row = summarize_group("EventClass", f"PriorNeutralPersistence {key[0]} + {key[1]}", grouped_persistence_destination[key])
        row["InteractionType"] = "PriorNeutralPersistenceGroup x DestinationFamily"
        rows.append(row)
...
def prior_persistence_values(events: Sequence[Event]) -> List[int]:
    return [
        event.row.prior_neutral_persistence_bars
        for event in events
        if event.row.prior_neutral_persistence_bars is not None
    ]


def find_rank(ranking_rows: Sequence[Dict[str, object]], event_class: str) -> Optional[Dict[str, object]]:
    for row in ranking_rows:
        if row.get("EventClass") == event_class:
            return row
    return None


def find_destination(destination_rows: Sequence[Dict[str, object]], destination_family: str) -> Optional[Dict[str, object]]:
    for row in destination_rows:
        if row.get("DestinationFamily") == destination_family:
            return row
    return None


def compare_replay_validator_03(
    ranking_rows: Sequence[Dict[str, object]],
    destination_rows: Sequence[Dict[str, object]],
) -> List[Dict[str, object]]:
    checks = [
        ("NeutralFormation + MixedStructure", "Interaction"),
        ("Black + ExhaustionPersistence", "Interaction"),
        ("NeutralFormation + RecoveryResolution", "Interaction"),
        ("LateNeutral + MixedStructure", "Interaction"),
        ("ReassertionProcessing", "Destination"),
    ]
    rows: List[Dict[str, object]] = []
    for label, check_type in checks:
        if check_type == "Interaction":
            match = find_rank(ranking_rows, label)
            rows.append(
                {
                    "ReplayValidator03Class": label,
                    "CheckType": check_type,
                    "PresentIn03C": bool(match),
                    "Rank03C": match.get("Rank") if match else "N/A",
                    "QualityScore03C": match.get("QualityScore") if match else None,
                    "Count03C": match.get("Count") if match else 0,
                }
            )
        else:
            match = find_destination(destination_rows, label)
            rows.append(
                {
                    "ReplayValidator03Class": label,
                    "CheckType": check_type,
                    "PresentIn03C": bool(match),
                    "Rank03C": "DestinationTable",
                    "QualityScore03C": None,
                    "Count03C": match.get("Count") if match else 0,
                }
            )
    return rows
...
def write_report(
    ...
    prior_values = prior_persistence_values(events)
    comparison_rows = compare_replay_validator_03(ranking_rows, destination_rows)
    persistence_7_10 = next((row for row in persistence_rows if row.get("PriorNeutralPersistenceGroup") == "7-10"), None)
    strongest_persistence = max(
        [row for row in persistence_rows if int(row.get("Count", 0)) >= 10 and row.get("AvgDR5") is not None],
        key=lambda row: float(row["AvgDR5"]),
        default=None,
    )
    strongest = strongest_dimension(
        [
            ("DestinationFamily", destination_rows),
            ("PriorNeutralPersistence", persistence_rows),
    ...
    lines: List[str] = []
    lines.append("APVA Replay Validator 03C - Corrected Event Quality")
    lines.append("=" * 78)
    lines.append("")
    lines.append("Purpose")
    lines.append("Rerun APVA event-quality analysis using corrected PriorNeutralPersistenceBars.")
    ...
    lines.append("SECTION 1 - Validation")
    lines.append("-" * 78)
    lines.append("PriorNeutralPersistenceBars column exists: true")
    lines.append(f"Min PriorNeutralPersistenceBars: {format_number(min(prior_values) if prior_values else None, 0)}")
    lines.append(f"Max PriorNeutralPersistenceBars: {format_number(max(prior_values) if prior_values else None, 0)}")
    lines.append(f"Mean PriorNeutralPersistenceBars: {format_number(mean(prior_values))}")
    lines.append(f"Median PriorNeutralPersistenceBars: {format_number(statistics.median(prior_values) if prior_values else None)}")
    lines.append("")
    lines.append("Event Quality Dimensions")
    lines.append("-" * 78)
    lines.append("Captured: DestinationFamily, FailureWarningScore, PreviousLoopPhase, VolumePolarity,")
    lines.append("PriorNeutralPersistenceBars, StructuralState, AgeBucket.")
    lines.append("")

    lines.append("SECTION 2 - Prior Neutral Persistence Study")
    lines.append("-" * 78)
    ...
    lines.append("SECTION 5 - Compare Against ReplayValidator03")
    lines.append("-" * 78)
    append_table(lines, comparison_rows)
    lines.append("")
    ...
    lines.append(
        "7-10 strongest persistence bucket: "
        + (
            "true"
            if strongest_persistence
            and strongest_persistence.get("PriorNeutralPersistenceGroup") == "7-10"
            else "false"
        )
    )
    if persistence_7_10:
        lines.append(
            "7-10 persistence bucket: "
            f"Count={persistence_7_10.get('Count')} "
            f"AvgDR5={format_number(persistence_7_10.get('AvgDR5'))} "
            f"WinRate5={format_number(persistence_7_10.get('WinRate5'))}"
        )
    if ranking_rows:
    ...
    lines.append("Corrected persistence materially changes quality rankings if prior-persistence interactions move into or out of the top classes.")
